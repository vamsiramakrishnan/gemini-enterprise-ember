  ...
  return (
    <div className="bg-gray-50 rounded-lg border border-gray-200 p-3 space-y-2.5">
      ...
      <div className="flex items-center gap-1.5">
        ...
        <span className="text-[11px] font-semibold" style={{ fontFamily: 'var(--font-ui)', color: 'var(--color-text-secondary)' }}>
          Context Strategy
        </span>
      </div>
      ...
      <div className="space-y-1">
        <div className="text-[10px] font-medium uppercase tracking-wider" style={{ color: 'var(--color-text-tertiary)' }}>Window</div>
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px]" style={{ color: 'var(--color-text-secondary)' }}>Last 3 turns</span>
          <span className="text-[10px] font-mono" style={{ color: 'var(--color-text-tertiary)' }}>C.window(3)</span>
        </div>
      </div>

      {/* State Injections */}
      <div className="space-y-1">
        <div className="text-[10px] font-medium uppercase tracking-wider" style={{ color: 'var(--color-text-tertiary)' }}>State Injections</div>
        ...
        <div className="text-[10px] font-mono" style={{ color: 'var(--color-text-tertiary)' }}>C.from_state(&quot;topic&quot;, &quot;jurisdiction&quot;, &quot;claim_id&quot;)</div>
      </div>

      {/* Filtering */}
      <div className="space-y-1">
        <div className="text-[10px] font-medium uppercase tracking-wider" style={{ color: 'var(--color-text-tertiary)' }}>Filtering</div>
        ...
        <div className="text-[10px] font-mono" style={{ color: 'var(--color-text-tertiary)' }}>C.user_only()</div>
      </div>

      {/* Summarization */}
      <div className="space-y-1">
        <div className="text-[10px] font-medium uppercase tracking-wider" style={{ color: 'var(--color-text-tertiary)' }}>Summarization</div>
        ...
        <div className="text-[10px] font-mono" style={{ color: 'var(--color-text-tertiary)' }}>C.summarize()</div>
      </div>

      {/* Annotation */}
      <div className="pt-1 border-t border-gray-200">
        <div className="text-[9px] leading-relaxed italic" style={{ color: 'var(--color-text-tertiary)' }}>
  ...
  return (
    <div>
      ...
      >
        ...
        <span style={{ color: 'var(--color-text-primary)' }}>{open ? 'Hide' : 'View'} Python Code</span>
        <span className="ml-auto">{open ? '\u25BE' : '\u25B8'}</span>
      </Button>
      {open && (
        <div className="mt-1.5 rounded-lg overflow-hidden border border-gray-200">
          {loading ? (
            <div className="px-3 py-3 text-[10px] text-center" style={{ color: 'var(--color-text-tertiary)' }}>Generating...</div>
          ) : code ? (
            <>
              ...
              <div className="px-2 py-1.5 bg-gray-50 border-t border-gray-200 flex items-center gap-1.5">
                <Button
                  variant="ghost"
                  size="sm"
                  className="bg-violet-100 text-violet-700 hover:bg-violet-200 text-[9px]"
                  onClick={() => {
                ...
                <span className="text-[9px] ml-auto" style={{ color: 'var(--color-text-tertiary)' }}>{code.dependencies.join(', ')}</span>
              </div>
            </>
          ) : (
            <div className="px-3 py-2 text-[10px]" style={{ color: 'var(--color-text-tertiary)' }}>Not available</div>
          )}
        </div>
      )}
    </div>
  ...
  return (
    <div className="p-4 space-y-4">
      <div>
        ...
        <div className="mt-2 text-xs" style={{ color: 'var(--color-text-secondary)' }}>{chip.registryId}</div>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-3 space-y-2">
        ...
        {chip.endpoint && (
          <div className="flex justify-between text-xs">
            ...
            <span className="font-mono text-[10px] truncate max-w-32" style={{ color: 'var(--color-text-primary)' }}>{chip.endpoint}</span>
          </div>
        )}
        {chip.healthStatus && (
          <div className="flex justify-between text-xs">
            ...
            <span className="flex items-center gap-1">
              ...
              <span style={{ color: 'var(--color-text-primary)' }}>{chip.healthStatus}</span>
            </span>
          </div>
        )}
      </div>
      ...
      <div>
        <div className="text-xs font-medium mb-1" style={{ color: 'var(--color-text-secondary)' }}>Description</div>
        <div className="text-xs leading-relaxed" style={{ color: 'var(--color-text-primary)' }}>{chip.description}</div>
      </div>
